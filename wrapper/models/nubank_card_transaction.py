...
class NuBankCardTransaction(BaseModel):
    ...
    def add_details_from_card_statement(self):
        ...
        statement = [statement for statement in card_statements if (
            self.transaction_id is not None and statement['id'] == self.transaction_id)]
...
class NuBankCardBillTransactions(BaseModel):
    ...
    def group_tags_and_get_amount_from_transactions(self) -> CardBillAmountPerTag:
        transactions_with_tag_obj = [
            transaction for transaction in self.transactions if len(transaction.tags) > 0]
    ...
    def __get_amount_per_tag(self, transactions_list: list[NuBankCardTransaction]) -> dict[str, float]:
        amount_per_tag_dict = {}
